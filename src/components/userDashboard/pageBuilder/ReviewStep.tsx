    ...
    <div className="space-y-6 max-w-xl mx-auto text-center">
      <div className={`p-8 rounded-lg ${bgMuted}`}>
        ...
        <h3 className={`text-xl font-serif font-semibold mb-2 ${textMuted}`}>
          Ready to Create Memorial
        </h3>
        <p className={`mb-6 ${textMuted}`}>
          Your beautiful memorial page is ready to be created. You can always edit and add more
          content later.
        </p>

        <div className="space-y-3 text-left">
          <div className="flex items-center justify-between">
            <span className={`text-sm ${textMuted}`}>Template:</span>
            <span className={`text-sm font-medium ${textMuted}`}>
              {templates.find((t) => t.id === selectedTemplate)?.name}
            </span>
          </div>
          <div className="flex items-center justify-between">
            <span className={`text-sm ${textMuted}`}>Privacy:</span>
            <span className={`text-sm font-medium ${textMuted}`}>Public</span>
          </div>
          <div className="flex items-center justify-between">
            <span className={`text-sm ${textMuted}`}>Features:</span>
            <span className={`text-sm font-medium ${textMuted}`}>Full Access</span>
          </div>
        </div>
      </div>
    </div>
