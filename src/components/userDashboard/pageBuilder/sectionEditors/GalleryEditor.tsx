...
export const GalleryEditor: React.FC<GalleryEditorProps> = ({ data, onChange }) => {
  ...
  const handleMediaSelect = (selectedMedia: UserUpload[]) => {
    console.log("GalleryEditor: Selected uploads:", selectedMedia);
    const newItems: GalleryItem[] = selectedMedia.map((upload) => ({
      id: upload.id,
      mediaId: upload.id,
      url: upload.url,
      caption: upload.title || upload.description || "",
      type: upload.type === "IMAGE" ? "image" : "video",
    }));
    ...
    console.log("GalleryEditor: Calling onChange with GALLERY data:", newData);
    onChange(newData);
    setIsMediaPickerOpen(false); // Close the local media picker
  };

  const handleOpenMediaPicker = () => {
    // Use only local media picker for gallery editor
    console.log("GalleryEditor: Opening media picker");
      ...
      <div className="space-y-6">
        <div>
          <h3 className="text-lg font-semibold mb-2">
            {t("dashboard.pageBuilder.gallery.title", {}, "Photo & Video Gallery")}
          </h3>
          <p className={`text-sm mb-4 ${textMuted}`}>
            {t(
              "dashboard.pageBuilder.gallery.description",
              {},
        ...
        {/* Layout Selection */}
        <div>
          <Label>{t("dashboard.pageBuilder.gallery.layout", {}, "Gallery Layout")}</Label>
        ...
        {/* Add Media Button */}
        <Button onClick={handleOpenMediaPicker} className="w-full" variant="outline">
          <Plus className="h-4 w-4 mr-2" />
          {t("dashboard.pageBuilder.gallery.addMedia", {}, "Add Photos/Videos from Gallery")}
        </Button>
          ...
          {data.items.length === 0 ? (
            ...
            >
              ...
              <p className={textMuted}>
                {t("dashboard.pageBuilder.gallery.noMedia", {}, "No media added yet")}
              </p>
              <p className={`text-xs mt-1 ${textMuted}`}>
                {t(
                  "dashboard.pageBuilder.gallery.clickToAdd",
                  {},
              ...
              >
                ...
                <div className="flex-1">
                  <Label htmlFor={`caption-${item.id}`}>
                    {t("dashboard.pageBuilder.gallery.caption", {}, "Caption (Optional)")}
                  </Label>
                  <input
                    id={`caption-${item.id}`}
                    type="text"
                    placeholder={t(
                      "dashboard.pageBuilder.gallery.captionPlaceholder",
                      {},
                      "Add a caption..."
                    )}
                    value={item.caption || ""}
                ...
                <button
                  ...
                  aria-label={t("dashboard.pageBuilder.gallery.remove", {}, "Remove media")}
                >
